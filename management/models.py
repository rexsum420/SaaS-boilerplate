...
class Manager(models.Model):
    ...
    def clock_in(self):
        ...
        if not ManagerEntry.objects.filter(employee=self, clock_out__isnull=True).exists():
            ManagerEntry.objects.create(employee=self, clock_in=timezone.now())
        else:
            raise ValueError("Employee is already clocked in.")

    def clock_out(self):
        # Update the existing open TimeEntry with clock_out time
        open_entry = ManagerEntry.objects.filter(employee=self, clock_out__isnull=True).first()
        if open_entry:
    ...
    def weekly_hours(self):
        ...
        weekly_entries = ManagerEntry.objects.filter(
            employee=self,
...
class ManagerEntry(models.Model):
    ...
    def __str__(self):
        return f"{self.manager.user.username} - {self.clock_in} to {self.clock_out}"
