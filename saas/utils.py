...
def get_safe_url(view_name, *args, **kwargs):
    try:
        # Attempt to resolve the URL based on the provided view name and arguments
        return reverse(view_name, args=args, kwargs=kwargs)
    except NoReverseMatch:
        # Handle unresolved URLs gracefully
...
def get_model_admin_url(model, action='changelist'):
    app_label = model._meta.app_label
    model_name = model._meta.model_name
    view_name = f'admin:{app_label}_{model_name}_{action}'

    # Use get_safe_url instead of reverse to handle unresolved URLs gracefully
    return get_safe_url(view_name)